...
if __name__ == "__main__":
    ...
    if args.method not in ["Manhattan", "Pattern Database", "Linear Conflict"]:
    ...
    if args.method == "Manhattan":
        s1 = time.time()
        chemin = A_search_manhatan(puzzle, s)
        e1 = time.time()
        s2 = time.time()
        chemin2 = A_search_manhatan_heap(puzzle, s)
        e2 = time.time()
        print(f"Normal manhatan {round(e1 - s1, 3)} Heap Manhatan {round(e2 - s2, 3)}")
    elif args.method == "Pattern Database":
        chemin = A_search_patern_data_heap(puzzle, s)
    elif args.method == "Linear Conflict":
        s1 = time.time()
        chemin = A_search_linear_confilct(puzzle, s)
        e1 = time.time()
        s2 = time.time()
        chemin2 = A_search_linear_confilct_heap(puzzle, s)
        e2 = time.time()
