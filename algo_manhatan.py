    ...
    while len(open_tab) > 0:
        ...
        for tab in open_tab:
            if Npuzzle.algo == "astar":
                f = g_values[tuple(tab)] + heuristic_manhattan(tab, Npuzzle.goal, Npuzzle.size)
            elif Npuzzle.algo == "uniform":
                f = g_values[tuple(tab)]
            elif Npuzzle.algo == "greedy":
                f = heuristic_manhattan(tab, Npuzzle.goal, Npuzzle.size)
            if f < f_min:
                f_min = f
                chosen_tab = tab
...
def A_search_manhatan_heap(Npuzzle):
    if Npuzzle.puzzle_resolve():
        print("Already solved !")
        return None
    ...
    f_start = heuristic_manhattan(Npuzzle.puzzle, Npuzzle.goal, Npuzzle.size)
        ...
        for pos_puzzle in possible_moves(chosen_tab, Npuzzle.size):
            if tuple(pos_puzzle) not in close_tab:
                ...
                if tuple(pos_puzzle) not in g_values or g_next < g_values[tuple(pos_puzzle)]:
                    ...
                    chemin[tuple(pos_puzzle)] = chosen_tab
                    if Npuzzle.algo == "astar":
                        f_next = g_next + heuristic_manhattan(pos_puzzle, Npuzzle.goal, Npuzzle.size)
                    elif Npuzzle.algo == "uniform":
                        f_next = g_next
                    elif Npuzzle.algo == "greedy":
                        f_next = heuristic_manhattan(pos_puzzle, Npuzzle.goal, Npuzzle.size)
